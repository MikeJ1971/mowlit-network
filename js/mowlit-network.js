...
function loadGraphData(graphId, onLoaded, onError) {
    const url = `./data/${graphId}.json`;
    jQuery.getJSON(url)
        .done((data) => {
        if (!data || typeof data !== 'object') {
            const message = `Graph data at ${url} is not a valid object.`;
        ...
        const message = `Failed to load graph data from ${url}: ${textStatus}`;
        console.error(message, errorThrown !== null && errorThrown !== void 0 ? errorThrown : jqXHR.statusText);
        onError === null || onError === void 0 ? void 0 : onError(message);
    });
}
...
function createGraph(graphId) {
    ...
    if (!graphContainer) {
        console.error(`Graph container #${GRAPH_CONTAINER_ID} was not found.`);
        return;
    }
    loadGraphData(graphId, (data) => {
        ...
        graphContainer.style.alignItems = 'top';
